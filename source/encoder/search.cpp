...
void Search::codeIntraLumaQT(Mode& mode, const CUGeom& cuGeom, uint32_t tuDepth, uint32_t absPartIdx, bool bAllowSplit, Cost& outCost, const uint32_t depthRange[2])
{
    ...
    if (mightNotSplit)
    {
        ...
        if (numSig)
        {
            ...
            primitives.pu[sizeIdx].copy_pp(reconQt, reconQtStride, pred, stride);
    ...
    primitives.pu[sizeIdx].copy_pp(picReconY, picStride, reconQt, reconQtStride);
...
void Search::codeIntraLumaTSkip(Mode& mode, const CUGeom& cuGeom, uint32_t tuDepth, uint32_t absPartIdx, Cost& outCost)
{
    ...
    for (int useTSkip = 0; useTSkip <= checkTransformSkip; useTSkip++)
    {
        ...
        else if (useTSkip)
        {
            ...
            primitives.pu[sizeIdx].copy_pp(tmpRecon, tmpReconStride, pred, stride);
    ...
    if (bTSkip)
    {
        ...
        primitives.pu[sizeIdx].copy_pp(reconQt, reconQtStride, tsReconY, tuSize);
    }
...
void Search::residualTransformQuantIntra(Mode& mode, const CUGeom& cuGeom, uint32_t absPartIdx, uint32_t tuDepth, const uint32_t depthRange[2])
{
    ...
    if (bCheckFull)
    {
        ...
        {
            primitives.pu[sizeIdx].copy_pp(picReconY, picStride, pred, stride);
...
uint32_t Search::codeIntraChromaQt(Mode& mode, const CUGeom& cuGeom, uint32_t tuDepth, uint32_t absPartIdx, uint32_t& psyEnergy)
{
    ...
    do
    {
        ...
        for (uint32_t chromaId = TEXT_CHROMA_U; chromaId <= TEXT_CHROMA_V; chromaId++)
        {
            ...
            {
                // no coded residual, recon = pred
                primitives.pu[sizeIdxC].copy_pp(reconQt, reconQtStride, pred, stride);
            ...
            primitives.pu[sizeIdxC].copy_pp(picReconC, picStride, reconQt, reconQtStride);
        }
    }
    ...
    do
    {
        ...
        for (uint32_t chromaId = TEXT_CHROMA_U; chromaId <= TEXT_CHROMA_V; chromaId++)
        {
            ...
            for (int useTSkip = 0; useTSkip <= checkTransformSkip; useTSkip++)
            {
                ...
                {
                    primitives.pu[sizeIdxC].copy_pp(recon, reconStride, pred, stride);
                    cu.setCbfPartRange(0, ttype, absPartIdxC, tuIterator.absPartIdxStep);
                }
            ...
            if (bTSkip)
            {
                ...
                primitives.pu[sizeIdxC].copy_pp(reconQt, reconQtStride, tskipReconC, MAX_TS_SIZE);
            }
            ...
            intptr_t picStride = m_frame->m_reconPic->m_strideC;
            primitives.pu[sizeIdxC].copy_pp(reconPicC, picStride, reconQt, reconQtStride);
...
void Search::residualQTIntraChroma(Mode& mode, const CUGeom& cuGeom, uint32_t absPartIdx, uint32_t tuDepth)
{
    ...
    do
    {
        ...
        for (uint32_t chromaId = TEXT_CHROMA_U; chromaId <= TEXT_CHROMA_V; chromaId++)
        {
            ...
            {
                // no coded residual, recon = pred
                primitives.pu[sizeIdxC].copy_pp(picReconC, picStride, pred, stride);
                cu.setCbfPartRange(0, ttype, absPartIdxC, tuIterator.absPartIdxStep);
            }
        }
    }
...
#define TRY_ANGLE(angle) \
    if (allangs) { \
        ...
        if (g_intraFilterFlags[angle] & scaleTuSize) \
            primitives.intra_pred[angle][sizeIdx](tmp, scaleTuSize, intraNeighbourBuf[1], angle, scaleTuSize <= 16); \
        else \
            primitives.intra_pred[angle][sizeIdx](tmp, scaleTuSize, intraNeighbourBuf[0], angle, scaleTuSize <= 16); \
...
//                        primitives.intra_pred[mode][sizeIdx](tmp, scaleTuSize, leftFiltered, aboveFiltered, mode, scaleTuSize <= 16);
                        primitives.intra_pred[mode][sizeIdx](tmp, scaleTuSize, intraNeighbourBuf[1], mode, scaleTuSize <= 16);
                    else
//                        primitives.intra_pred[mode][sizeIdx](tmp, scaleTuSize, left, above, mode, scaleTuSize <= 16);
                        primitives.intra_pred[mode][sizeIdx](tmp, scaleTuSize, intraNeighbourBuf[0], mode, scaleTuSize <= 16);
                    bits = (mpms & ((uint64_t)1 << mode)) ? m_entropyCoder.bitsIntraModeMPM(preds, mode) : rbits;
                    sad = sa8d(fenc, scaleStride, tmp, scaleTuSize) << costShift;
                    modeCosts[mode] = m_rdCost.calcRdSADCost(sad, bits);
                    COPY1_IF_LT(bcost, modeCosts[mode]);
        ...
        if (puIdx != numPU - 1)
        {
            ...
            primitives.pu[log2TrSize - 2].copy_pp(dst, dststride, src, srcstride);
        }
    }
