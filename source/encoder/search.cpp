...
uint32_t Search::codeIntraChromaQt(Mode& mode, const CUGeom& cuGeom, uint32_t trDepth, uint32_t absPartIdx, uint32_t& psyEnergy)
{
    ...
    for (uint32_t chromaId = TEXT_CHROMA_U; chromaId <= TEXT_CHROMA_V; chromaId++)
    {
        ...
        do
        {
            ...
            int16_t* reconQt       = m_rqt[qtLayer].reconQtYuv.getChromaAddr(chromaId, absPartIdxC);
            ...
            intptr_t reconPicStride = m_frame->m_reconPicYuv->m_strideC;
            ...
            if (numSig)
            {
                ...
                primitives.calcrecon[sizeIdxC](pred, residual, reconQt, reconPicC, stride, reconQtStride, reconPicStride);
                tmpDist = primitives.sse_sp[sizeIdxC](reconQt, reconQtStride, fenc, stride);
            ...
            else
            {
                ...
                primitives.square_copy_pp[sizeIdxC](reconPicC, reconPicStride, pred, stride);
                tmpDist = primitives.sse_pp[sizeIdxC](pred, stride, fenc, stride);
            ...
            if (m_rdCost.m_psyRd)
                psyEnergy += m_rdCost.psyCost(sizeIdxC, fenc, stride, reconPicC, reconPicStride);
        }
...
uint32_t Search::codeIntraChromaTSkip(Mode& mode, const CUGeom& cuGeom, uint32_t trDepth, uint32_t absPartIdx, uint32_t& psyEnergy)
{
    ...
    X265_CHECK(cu.m_trIdx[absPartIdx] == trDepth && 2 == log2TrSizeC && MAX_TS_SIZE == tuSize, "transform skip only possible at 4x4\n");
    ...
    ALIGN_VAR_32(int16_t, tskipReconC[MAX_TS_SIZE * MAX_TS_SIZE]);
    ...
    for (uint32_t chromaId = TEXT_CHROMA_U; chromaId <= TEXT_CHROMA_V; chromaId++)
    {
        ...
        do
        {
            ...
            for (int useTSkip = 0; useTSkip <= checkTransformSkip; useTSkip++)
            {
                ...
                int16_t* recon = (useTSkip ? tskipReconC : reconQt);
                ...
                if (numSig)
                {
                    ...
                    primitives.calcrecon[sizeIdxC](pred, residual, recon, reconPicC, stride, reconStride, reconPicStride);
                    tmpDist = primitives.sse_sp[sizeIdxC](reconQt, reconQtStride, fenc, stride);
                    cu.setCbfPartRange(1 << trDepth, ttype, absPartIdxC, tuIterator.absPartIdxStep);
                }
                else if (useTSkip)
                ...
                if (m_rdCost.m_psyRd)
                {
                    tmpEnergy = m_rdCost.psyCost(sizeIdxC, fenc, stride, reconPicC, reconPicStride);
                    tmpCost = m_rdCost.calcPsyRdCost(tmpDist, tmpBits, tmpEnergy);
                }
                else
                    tmpCost = m_rdCost.calcRdCost(tmpDist, tmpBits);
            ...
            if (bTSkip)
            {
                ...
                primitives.square_copy_ss[sizeIdxC](reconQt, reconQtStride, tskipReconC, MAX_TS_SIZE);
            }
            else if (checkTransformSkip)
                primitives.square_copy_sp[sizeIdxC](reconPicC, reconPicStride, reconQt, reconQtStride);
                                     ...
                                     uint32_t& outBits, uint32_t depthRange[2])
{
    ...
    if (bCheckFull)
    {
        ...
        m_entropyCoder.resetBits();
        m_entropyCoder.codeQtCbf(cbfFlag[TEXT_LUMA][0], TEXT_LUMA, trMode);
        ...
        if (bCodeChroma)
        {
            ...
            do
            {
                ...
                m_entropyCoder.codeQtCbf(cbfFlag[TEXT_CHROMA_U][tuIterator.section], TEXT_CHROMA_U, trMode);
                ...
                m_entropyCoder.codeQtCbf(cbfFlag[TEXT_CHROMA_V][tuIterator.section], TEXT_CHROMA_V, trMode);
        ...
        if (bCodeChroma)
        {
            ...
            do
            {
                ...
                cu.setCbfPartRange(cbfFlag[TEXT_CHROMA_U][tuIterator.section] << trMode, TEXT_CHROMA_U, absPartIdxC, tuIterator.absPartIdxStep);
                cu.setCbfPartRange(cbfFlag[TEXT_CHROMA_V][tuIterator.section] << trMode, TEXT_CHROMA_V, absPartIdxC, tuIterator.absPartIdxStep);
            }
            while (tuIterator.isNextSection());
        }
        ...
        if (checkTransformSkipY)
        {
            ...
            {
                m_entropyCoder.resetBits();
                m_entropyCoder.codeQtCbf(!!numSigTSkipY, TEXT_LUMA, trMode);
        ...
        if (bCodeChroma && checkTransformSkipUV)
        {
            ...
            do
            {
                ...
                if (numSigTSkipU)
                {
                    m_entropyCoder.codeQtCbf(!!numSigTSkipU, TEXT_CHROMA_U, trMode);
                ...
                if (numSigTSkipV)
                {
                    m_entropyCoder.codeQtCbf(!!numSigTSkipV, TEXT_CHROMA_V, trMode);
        ...
        if (bCodeChroma)
        {
            for (uint32_t chromaId = TEXT_CHROMA_U; chromaId <= TEXT_CHROMA_V; chromaId++)
            {
                if (splitIntoSubTUs)
                {
                    offsetSubTUCBFs(cu, (TextType)chromaId, trMode, absPartIdx);
                    for (uint32_t subTU = 0; subTU < 2; subTU++)
                        m_entropyCoder.codeQtCbf(cbfFlag[chromaId][subTU], (TextType)chromaId, trMode);
                }
                else
                    m_entropyCoder.codeQtCbf(cbfFlag[chromaId][0], (TextType)chromaId, trMode);
            }
        }
        ...
        for (uint32_t chromaId = TEXT_CHROMA_U; chromaId <= TEXT_CHROMA_V; chromaId++)
        {
            for (uint32_t subTUIndex = 0; subTUIndex < numberOfSections; subTUIndex++)
            {
                ...
                if (splitIntoSubTUs)
                {
                    ...
                    cu.setCbfPartRange(((cbfFlag[chromaId][subTUIndex] << 1) | combinedSubTUCBF) << trMode, (TextType)chromaId, subTUPartIdx, partIdxesPerSubTU);
                }
                else
                    cu.setCbfPartRange(cbfFlag[chromaId][subTUIndex] << trMode, (TextType)chromaId, subTUPartIdx, partIdxesPerSubTU);
            }
        }
    }
...
void Search::saveResidualQTData(CUData& cu, ShortYuv& resiYuv, uint32_t absPartIdx, uint32_t depth)
{
    ...
    if (bCodeChroma)
    {
        m_rqt[qtLayer].resiQtYuv.copyPartToPartChroma(resiYuv, absPartIdx, log2TrSizeC + hChromaShift);
